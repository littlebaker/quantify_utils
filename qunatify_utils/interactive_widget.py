...
from typing import Sequence, Literal, Iterable, Callable, Any
from pydantic import NoneBytes
from quantify_core.data.experiment import QuantifyExperiment
from ipywidgets import VBox, HBox, Box, HTML, Output, \
    Tab, Button, Layout, Dropdown, Textarea, GridspecLayout, Label
import panel as pn
...
def _plot_ds(ds: Dataset) -> None:
    ...
    plot = df.hvplot.explorer(x='x0', y='y0') # might fail
    return plot

def _do_in_tab(
    tab: Tab, ds: QuantifyExperiment, which: Literal["plot", "snapshot"]
) -> Callable[[Button], None]:
    ...
    def change_figure_type(change):
        pass

    def _on_click(_: Button) -> None:
        assert which in ("plot", "snapshot")
        title = f"RID #{ds.tuid} {which}"
        i = next(
        ...
        with out:
            ...
            display(HBox([close_button]))
        tab.selected_index = i

    return _on_click

def _get_plot_button(ds: QuantifyExperiment, tab: Tab) -> Button:
    return button(
        "",
        "warning",
...
def _get_run_id_button(e: QuantifyExperiment, index: int) -> Box:
    title = str(index)
    body = yaml.dump(
        {
            ".tuid": e.tuid,
...
def button(
    ...
    button_kwargs: dict[str, Any] = None,
) -> Button:
...
def _experiment_widget(
    exp_sets: Iterable[QuantifyExperiment], tab: Tab
):
    r"""Show a `ipywidgets.GridspecLayout` with information about the
    ...
    for index, e in enumerate(exp_sets):
        dset = e.load_dataset()
        snapshot = e.load_dataset()
        row = {}
        row['ID'] = _get_run_id_button(e, index)
        row["Datetime"] = label(datetime.datetime.strptime(e.tuid[:18], "%Y%m%d-%H%M%S-%f").strftime("%Y/%m/%d-%H:%M:%S"))
        row['Parameters'] = label("To be added")
        row['Snapshot'] = label("TODO")
        row["Plot"] = _get_plot_button(dset, tab)
        rows.append(row)
...
def get_date_dirs(data_dir: str):
    abs_path = os.path.abspath(data_dir)
    dir_list = os.listdir(abs_path)
    
    data_date_dir = []
    for dirname in dir_list:
        i = re.match("[\d]{8}", dirname)
        if i is not None:
            data_date_dir.append(i.group(0))
    return data_date_dir
...
def experiments_widget(
    ...
    data_date_dirs = get_date_dirs(data_dir)
    dropdown_widget = Dropdown(
        options=data_date_dirs,
    )
    
    if len(data_date_dirs) != 0:
        assert date in data_date_dirs, f"date must be in {data_date_dirs}"
        exp_sets = get_experiments(os.path.join(data_dir, date))
    # TODO: add logic
    ...
    def on_date_change(change):
        ...
        if change['name'] == "value":
            exp_sets = get_experiments(os.path.join(data_dir, change['new']))
    dropdown_widget.observe(on_date_change)
            
    # dropdown_box = HBox([
    #     HTML("<h3>Please select a date:  </h3>"), dropdown_widget
    # ])
    # if sort_by == "run_id":
    ...
    grid = _experiment_widget(exp_sets, tab)
